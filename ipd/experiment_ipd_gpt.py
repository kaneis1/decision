    ...
    def evaluate_gpt2_ipd_metrics(
        ...
        with torch.no_grad():
            for traj_idx, traj in enumerate(trajectories):
                ...
                logits = model(states_tensor, attention_mask=attention_mask_4d)  # [1, T, 1]
                probs = torch.sigmoid(logits.squeeze(-1))  # [1, T] - probability of action=1
                p_action1 = probs[0, :].cpu().numpy()  # [T] - probability of action=1
    ...
    def train_single_fold(fold_idx: int, train_inds: np.ndarray, val_inds: np.ndarray) -> tuple[float | None, object]:
        ...
        class StateGPT2Model(torch.nn.Module):
            def __init__(self, state_dim, embed_dim, n_layer, n_head, n_inner, activation_function, dropout, n_positions):
                ...
                self.action_head = torch.nn.Linear(embed_dim, 1)  # Binary classification: single logit
            ...
            def forward(self, states, attention_mask=None):
                ...
                # Predict actions
                logits = self.action_head(outputs.last_hidden_state)  # [B, T, 1]
                return logits
        ...
        def compute_val_loss():
            ...
            with torch.no_grad():
                ...
                for i in range(0, len(val_states_tensor), val_batch_size):
                    ...
                    logits = model(states_batch, attention_mask=attention_mask)  # [B, T, 1]
                    logits = logits.view(-1, 1)  # [B*T, 1]
                    labels = actions_batch.contiguous().view(-1, 1).float()  # [B*T, 1] - float for BCE
        ...
        def compute_val_accuracy():
            ...
            with torch.no_grad():
                ...
                for i in range(0, len(val_states_tensor), val_batch_size):
                    ...
                    logits = model(states_batch, attention_mask=attention_mask)  # [B, T, 1]
                    logits = logits.view(-1, 1)  # [B*T, 1]
                    labels = actions_batch.contiguous().view(-1, 1).float()  # [B*T, 1]
                    mask_flat = (masks_batch.view(-1) == 1)
