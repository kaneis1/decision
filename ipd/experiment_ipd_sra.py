# experiment_ipd_sra.py
# Train a Decision Transformer (S, R, A ordering) on Iterated Prisoner's Dilemma CSV (all_data.csv)
#
# This uses DecisionTransformerSRA which orders tokens as (state, return, action) instead of (return, state, action)
...
from decision_transformer.models.decision_transformer_sra import DecisionTransformerSRA
...
def main(args):
    ...
    def train_single_fold(fold_idx: int, train_inds: np.ndarray, val_inds: np.ndarray) -> tuple[float | None, object]:
        ...
        # Use DecisionTransformerSRA instead of DecisionTransformer
        model = DecisionTransformerSRA(
            state_dim=state_dim,
            act_dim=act_dim,
            max_length=K,
